...
func TestExportThenInitGenesis(t *testing.T) {
	oldKeeper, _, oldCtx := testkeeper.DymNSKeeper(t)
	...
	opo2 := dymnstypes.OpenPurchaseOrder{
		...
		ExpireAt:  1,
		MinPrice:  sdk.NewCoin(params.BaseDenom, sdk.NewInt(100)),
		SellPrice: sdk.NewCoin(params.BaseDenom, sdk.NewInt(900)),
		HighestBid: &dymnstypes.OpenPurchaseOrderBid{
			Bidder: bidder2,
			Price:  sdk.NewCoin(params.BaseDenom, sdk.NewInt(800)),
		},
	}
	...
	opo3 := dymnstypes.OpenPurchaseOrder{
		...
		ExpireAt:  1,
		MinPrice:  sdk.NewCoin(params.BaseDenom, sdk.NewInt(100)),
		SellPrice: sdk.NewCoin(params.BaseDenom, sdk.NewInt(200)),
	}
	...
	t.Run("dym-names should be imported correctly", func(t *testing.T) {
		...
		require.Len(t, newDymNsKeeper.GetAllNonExpiredDymNames(newCtx, time.Now().UTC().Unix()), 2)
	...
	t.Run("open purchase orders's non-refunded bids should be refunded correctly", func(t *testing.T) {
		require.Equal(t,
			sdk.NewCoin(params.BaseDenom, sdk.NewInt(200)),
			newBankKeeper.GetBalance(newCtx, sdk.MustAccAddressFromBech32(bidder1), params.BaseDenom),
		)
		require.Equal(t,
			sdk.NewCoin(params.BaseDenom, sdk.NewInt(800)),
