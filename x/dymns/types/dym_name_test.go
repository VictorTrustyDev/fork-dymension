...
func TestDymName_GetAddressesForReverseMapping(t *testing.T) {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantPanic {
				require.Panics(t, func() {
					_, _ = m.GetAddressesForReverseMapping(func(_ string) bool {
						return true
					})
				})
			...
			gotConfiguredAddresses, gotCoinType60HexAddresses := m.GetAddressesForReverseMapping(useFuncCheckChainIdIsCoinType60)
	...
	t.Run("func check chain-id is coin-type 60 chain is required", func(t *testing.T) {
		...
		require.Panics(t, func() {
			_, _ = dymName.GetAddressesForReverseMapping(nil)
		})
	})
}
