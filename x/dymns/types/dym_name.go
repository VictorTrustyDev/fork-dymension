...
func (m *DymName) Validate() error {
	...
	for _, config := range m.Configs {
		...
		configIdentity := strings.ToLower(fmt.Sprintf("%s|%s", config.Type, config.Path))
...
func (m *DymNameConfig) Validate() error {
	...
	if m.Value == "" {
		return ErrValidationFailed.Wrap("dym name config value is empty")
	}
