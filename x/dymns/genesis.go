...
func InitGenesis(ctx sdk.Context, k dymnskeeper.Keeper, genState dymnstypes.GenesisState) {
	...
	for _, bid := range genState.OpenPurchaseOrderBids {
...
func ExportGenesis(ctx sdk.Context, k dymnskeeper.Keeper) *dymnstypes.GenesisState {
	epochUtcNow := time.Now().UTC().Unix()

	var nonRefundedBids []dymnstypes.OpenPurchaseOrderBid
	for _, bid := range k.GetAllOpenPurchaseOrders(ctx) {
		if bid.HighestBid == nil {
			continue
		}
		// we ignore check expiry here because as long as OPO exists, the highest bid not processed yet.
	...
	return &dymnstypes.GenesisState{
		...
		OpenPurchaseOrderBids: nonRefundedBids,
	}
}
