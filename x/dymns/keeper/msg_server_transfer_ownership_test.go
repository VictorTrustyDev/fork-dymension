...
func Test_msgServer_TransferOwnership(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "fail - reject if Open Purchase Order exists, expired OPO",
			dymName: &dymnstypes.DymName{
				Owner:      owner,
				Controller: owner,
				ExpireAt:   now.Unix() + 1,
			...
			wantErr:         true,
			wantErrContains: "can not transfer ownership while there is an Open Purchase Order",
		},
		{
			name: "fail - reject if Open Purchase Order exists, not finished OPO",
			dymName: &dymnstypes.DymName{
				Owner:      owner,
				Controller: owner,
				ExpireAt:   now.Unix() + 1,
		...
		{
			name: "fail - reject if Open Purchase Order exists, completed OPO",
			dymName: &dymnstypes.DymName{
				Owner:      owner,
				Controller: owner,
				ExpireAt:   now.Unix() + 1,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.dymName != nil {
				...
				require.NoError(t, dk.SetOpenPurchaseOrder(ctx, *opo))

				err := dk.MoveOpenPurchaseOrderToHistorical(ctx, recordName)
				require.NoError(t, err)

				require.NotEmpty(t, dk.GetHistoricalOpenPurchaseOrders(ctx, recordName))
			}

			if tt.opo != nil {
				require.NotNil(t, tt.dymName, "bad test setup")
				tt.opo.Name = recordName
				require.NoError(t, dk.SetOpenPurchaseOrder(ctx, *tt.opo))
			}
			...
			if tt.wantErr {
				...
				if tt.dymName != nil {
					...
					if tt.dymName.ExpireAt > now.Unix() {
						...
						// GetDymNamesOwnedBy does not returns expired Dym-Names
						require.NoError(t, err)
						require.Len(t, list, 1, "reverse mapping should be kept")
					}

					require.NotEmpty(t, dk.GetHistoricalOpenPurchaseOrders(ctx, recordName), "historical OPO should be kept")
				}
				return
			}
			...
			require.Empty(t, dk.GetHistoricalOpenPurchaseOrders(ctx, recordName), "historical OPO should be removed")
		})
	}
}
