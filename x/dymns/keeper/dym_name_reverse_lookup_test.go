...
func TestKeeper_GetAddReverseMappingConfiguredAddressToDymName(t *testing.T) {
	...
	t.Run("allow Interchain Account (32 bytes)", func(t *testing.T) {
		require.NoError(
			t,
			dk.AddReverseMappingConfiguredAddressToDymName(ctx, interchainAccountBech32Addr, dymName1.Name),
		)

		linked3, err := dk.GetDymNamesContainsConfiguredAddress(ctx, interchainAccountBech32Addr, 0)
		require.NoError(t, err)
...
func TestKeeper_RemoveReverseMappingConfiguredAddressToDymName(t *testing.T) {
	...
	t.Run("remove correctly with Interchain Account (32 bytes)", func(t *testing.T) {
		...
		require.NoError(
			t,
			dk.RemoveReverseMappingConfiguredAddressToDymName(ctx, interchainAccountBech32Addr, dymName1.Name),
		)

		linked, err = dk.GetDymNamesContainsConfiguredAddress(ctx, interchainAccountBech32Addr, 0)
		require.NoError(t, err)
		require.Empty(t, linked)
	})
}
...
func TestKeeper_GetAddReverseMapping0xAddressToDymName(t *testing.T) {
	...
	pseudoBech32Addr := "dym1zg69v7yszg69v7yszg69v7yszg69v7ys8xdv96"
	pseudo0xAddr := sdk.MustAccAddressFromBech32(pseudoBech32Addr)
	require.Len(t, pseudo0xAddr.Bytes(), 20)
	...
	require.NoError(t, dk.SetDymName(ctx, dymName1))
	err := dk.AddReverseMapping0xAddressToDymName(ctx, pseudo0xAddr, dymName1.Name)
	require.NoError(t, err)
	...
	require.NoError(t, dk.SetDymName(ctx, dymName3))
	err = dk.AddReverseMapping0xAddressToDymName(ctx, pseudo0xAddr, dymName3.Name)
	require.NoError(t, err)

	require.NoError(
		t,
		dk.AddReverseMapping0xAddressToDymName(ctx, pseudo0xAddr, "not-exists"),
		"no check non-existing dym-name",
	)
	...
	linked1, err1 := dk.GetDymNamesContains0xAddress(ctx, pseudo0xAddr, 0)
	require.NoError(t, err1)
	require.Len(t, linked1, 2)
	requireEqualsStrings(t,
		[]string{dymName1.Name, dymName3.Name},
...
func TestKeeper_RemoveReverseMapping0xAddressToDymName(t *testing.T) {
	...
	require.NoError(t, dk.SetDymName(ctx, dymName2))
	err = dk.AddReverseMapping0xAddressToDymName(ctx, pseudo0xAddr, dymName2.Name)
	require.NoError(t, err)
	...
	linked, err := dk.GetDymNamesContains0xAddress(ctx, pseudo0xAddr, 0)
	require.NoError(t, err)
	require.Len(t, linked, 2, "existing data must be kept")
	...
	t.Run("no error if element is not in the list", func(t *testing.T) {
		require.NoError(
			t,
			dk.RemoveReverseMapping0xAddressToDymName(ctx, pseudo0xAddr, "aaaaa"),
		)
		linked, err = dk.GetDymNamesContains0xAddress(ctx, pseudo0xAddr, 0)
		require.NoError(t, err)
		require.Len(t, linked, 2, "existing data must be kept")
	...
	t.Run("remove correctly", func(t *testing.T) {
		require.NoError(
			t,
			dk.RemoveReverseMapping0xAddressToDymName(ctx, pseudo0xAddr, dymName1.Name),
		)
		...
		require.NoError(
			t,
			dk.RemoveReverseMapping0xAddressToDymName(ctx, pseudo0xAddr, dymName2.Name),
		)
