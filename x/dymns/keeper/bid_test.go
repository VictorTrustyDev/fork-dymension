...
func TestKeeper_RefundBid(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			refundAmount:    sdk.NewCoin(params.BaseDenom, sdk.NewInt(100)),
			genesis:         false,
			wantErr:         true,
		...
		},
		{
			...
			refundToAccount:          "dym1fl48vsnmsdzcv85q5d2q4z5ajdha8yu38x9fue",
			refundAmount:             sdk.NewCoin(params.BaseDenom, sdk.NewInt(100)),
			fundModuleAccountBalance: sdk.NewCoin(params.BaseDenom, sdk.NewInt(50)),
			genesis:                  false,
			wantErr:                  true,
			wantErrContains:          "insufficient funds",
		},
		{
			...
			refundToAccount:          "dym1fl48vsnmsdzcv85q5d2q4z5ajdha8yu38x9fue",
			refundAmount:             sdk.NewCoin(params.BaseDenom, sdk.NewInt(100)),
			fundModuleAccountBalance: sdk.NewCoin(params.BaseDenom, sdk.NewInt(150)),
			genesis:                  false,
		},
		{
		...
		},
		{
			...
			refundAmount:             sdk.NewCoin(params.BaseDenom, sdk.NewInt(100)),
			fundModuleAccountBalance: sdk.NewCoin(params.BaseDenom, sdk.NewInt(100)),
			wantErr:                  true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dk, bk, ctx := testkeeper.DymNSKeeper(t)
			...
			opoBid := dymnstypes.OpenPurchaseOrderBid{
			...
			var err error
			if tt.genesis {
				err = dk.GenesisRefundBid(ctx, opoBid)
			} else {
				err = dk.RefundBid(ctx, opoBid)
			}
