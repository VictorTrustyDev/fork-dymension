...
func Test_epochHooks_BeforeEpochStart(t *testing.T) {
	...
	setupTest := func() (dymnskeeper.Keeper, sdk.Context) {
		...
		params.Misc.DaysPreservedClosedSellOrder = daysKeepHistorical
...
func Test_epochHooks_AfterEpochEnd(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mapExpiryByDymName: map[string]int64{
				dymNameA.Name: soExpiredEpoch,
				dymNameB.Name: soNotExpiredEpoch,
				dymNameC.Name: soExpiredEpoch,
				dymNameD.Name: soExpiredEpoch,
			},
		...
		},
		{
			...
			mapExpiryByDymName: map[string]int64{
				dymNameA.Name: soExpiredEpoch,
				dymNameB.Name: soExpiredEpoch, // no SO for Dym-Name B but still have reference
			},
			wantErr:                false,
			wantMapExpiryByDymName: map[string]int64{
		...
		},
		{
			...
			mapExpiryByDymName: map[string]int64{
				dymNameA.Name: soExpiredEpoch,
				dymNameB.Name: soExpiredEpoch, // incorrect, SO not expired
			},
			wantErr: false,
			wantMapExpiryByDymName: map[string]int64{
				dymNameB.Name: soNotExpiredEpoch, // updated
			},
			afterHookTestFunc: func(t *testing.T, dk dymnskeeper.Keeper, bk dymnskeeper.BankKeeper, ctx sdk.Context) {
				// no extra test
			},
