...
func (suite *ChainIntegrationTestSuite) TxSendViaEVM(from, to *itutiltypes.TestAccount, amount float64) error {
	_, err := suite.DeliverEthTx(from, suite.buildMsgEthereumTxTransfer(from, to, amount))
	return err
}

// TxSendViaEVMAsync is the same as TxSendViaEVM but with Async delivery mode.
func (suite *ChainIntegrationTestSuite) TxSendViaEVMAsync(from, to *itutiltypes.TestAccount, amount float64) error {
	_, err := suite.DeliverEthTx(from, suite.buildMsgEthereumTxTransfer(from, to, amount))
	return err
}
